...
def get_agent(agent_id: int, session: Session = Depends(get_cca_session)):
...
def create_agent(agent: AgentCreate, session: Session = Depends(get_cca_session)):
...
def update_all_agents(session: Session = Depends(get_cca_session)):
    agents = session.query(AgentModel).all()
